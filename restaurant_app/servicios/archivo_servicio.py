...
# Relaciona el campo "tipo" guardado en el JSON con la clase que debe reconstruirse. Si en el futuro se agrega un nuevo tipo de 
# producto (por ejemplo Postre), bastaria con registrar su clase aqui (principio OCP). 
...
class ArchivoServicio:
    ...
    def cargar_productos(self) -> list[Producto]:
        """
        Lee datos/productos.json y reconstruye la coleccion de objetos Producto (y Bebida, cuando corresponda). Controla de forma 
        especifica los problemas que puede presentar el archivo, sin detener la aplicacion.
        """
    ...
    def guardar_productos(self, productos: list[Producto]) -> bool:
        """
        Convierte la coleccion de objetos Producto/Bebida a una lista de diccionarios (mediante convertir_a_diccionario(), que ahora 
        incluye el stock) y la guarda en datos/productos.json con json.dump().
        """
    ...
    def _reconstruir_producto(self, registro: dict) -> Producto:
        """
        Reconstruye un objeto Producto o Bebida a partir de un registro leido desde JSON, usando el campo "tipo" para elegir la clase 
        correcta. Puede lanzar KeyError (clave faltante) o ValueError (dato invalido, propagado desde los setters de Producto/Bebida); 
        ambas excepciones son controladas por quien llama a este metodo (cargar_productos()).
        """
    ...
    def cargar_clientes(self) -> list[Cliente]:
        """
        MEJORA SEMANA 11: completa la persistencia de clientes, que en la Semana 10 solo se administraban en memoria. Lee usuarios.json 
        y reconstruye objetos Cliente.
        """
    ...
    def cargar_ventas(self) -> list[Venta]:
        """
        MEJORA SEMANA 11: reconstruye las ventas ya realizadas para que, al reiniciar el programa, la consulta de ventas por usuario 
        siga funcionando sobre datos reales.
        """
