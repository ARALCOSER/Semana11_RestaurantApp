class Producto:
    """
    Clase base que representa un producto general del restaurante. Define los atributos obligatorios de Producto (codigo, nombre, categoria,
    precio y stock) y expone metodos para acceder y modificar dichos atributos de forma controlada.
    PRINCIPIO SRP (Responsabilidad Unica): su unica responsabilidad es modelar y exponer los datos propios del producto. No conoce nada sobre 
    el registro de productos, la persistencia en archivos ni la interaccion por consola. PRINCIPIO OCP (Abierto/Cerrado): permite extender 
    el catalogo a nuevos tipos de productos (como Bebida) mediante herencia, sin modificar la logica ya existente del servicio Restaurante ni 
    de ArchivoServicio.

    MEJORA SEMANA 11: se incorpora el atributo "stock" para poder representar la cantidad disponible de cada producto y sostener la nueva 
    operacion de venta (Cliente + Producto -> Venta).
    """

    # Identifica el tipo de producto dentro del JSON para poder reconstruirlo correctamente (Producto o Bebida) al cargar datos/productos.json.
    ...
    def mostrar_informacion(self) -> str:
        ...
        cualquier clase hija (por ejemplo Bebida) puede sustituir a Producto aqui sin alterar el comportamiento esperado por quien llama 
        al metodo.
        """
        return (
    ...
    def convertir_a_diccionario(self) -> dict:
        ...
        Se incluye el campo "tipo" para que, al reconstruir la coleccion con json.load(), cada registro pueda volver a convertirse en el 
        objeto correcto (Producto o Bebida).
        """
        return {
            "tipo": self.TIPO,
            "codigo": self.codigo,
